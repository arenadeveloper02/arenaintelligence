...
export default async function ContentResearchPage() {
  ...
  const [hasApiKey, executions] = await Promise.all([
    ...
    getAgentExecutions(user.id, 'content-research'),
  ])
  return (
    <AppShell user={user} active="content-research">
      <AgentRunnerClient agent={AGENTS['content-research']} hasApiKey={hasApiKey} executions={executions} />
    </AppShell>
  )
}
